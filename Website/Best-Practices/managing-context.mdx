...
**What the model can hold.** Every AI tool has a context window — a fixed number of [tokens](/Learn-About-AI/tokens) it can process at one time. Think of it as the model's short-term memory. Everything in your conversation — your prompts, the AI's responses, uploaded documents — has to fit inside this window. Once it fills up, the oldest information gets quietly dropped and the model loses track of what you told it earlier.
